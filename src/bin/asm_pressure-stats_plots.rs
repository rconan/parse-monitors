...
fn main() -> Result<()> {
    let mirror = "m2";
    let _ = cfd::Baseline::<2021>::at_zenith(cfd::ZenithAngle::Thirty)
        .into_iter()
        .collect::<Vec<cfd::CfdCase<2021>>>()
        .into_par_iter()
        .map(|data_path| {
            let mut df = {
                let filename = format!("{}_pressure-stats_within.csv", mirror);
            ...
            let filename = "m2_pressure-stats_std_within.png";
            let path = cfd::Baseline::<2021>::path()
                .join(data_path.to_string())
                .join(filename);
            ...
            let cols: Vec<_> = vec!["Var [Pa]".to_string()];
