...
use parse_monitors::{cfd, cfd::BaselineTrait, Band, DomeSeeing, CFD_YEAR};
use rayon::prelude::*;
use std::env;
...
fn main() -> anyhow::Result<()> {
    ...
    let cfd_cases_21 = cfd::Baseline::<CFD_YEAR>::default()
        //.extras()
        .into_iter()
        .skip(20)
        .take(5)
        .collect::<Vec<cfd::CfdCase<CFD_YEAR>>>();
    let root = cfd::Baseline::<CFD_YEAR>::path()?;
    let legends = format!("{CFD_YEAR}");
    let other_legends = format!("{OTHER_YEAR}");
        ...
        .map(|cfd_case_current| {
            ...
            if let (Some(v_pssn), Some(h_pssn)) = (ds_21.pssn(Band::V), ds_21.pssn(Band::H)) {
                ...
                Some((
                    ...
                    if let Some(cfd_case_other) =
                        cfd::Baseline::<OTHER_YEAR>::find(cfd_case_current)
                    {
                        let path = cfd::Baseline::<OTHER_YEAR>::path()
                            .ok()?
                            ...
                            (ds_20.pssn(Band::V), ds_20.pssn(Band::H))
                        {
                            ...
                            make_figure(
                                ...
                                pssn_labels
                                    .as_ref()
                                    .unwrap_or(&vec!["CFD_YEAR (SE)", "CFD_YEAR (LE)", "2020 (LE)"])
                                    .to_vec(),
                                path_to_case
                                    .join("report")
    ...
    println!(
        "CFD CFD_YEAR median PSSn V: {:.4}, H: {:.4}",
        med(&v_21),
        med(&h_21)
    ); */
    Ok(())
